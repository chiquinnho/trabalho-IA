# algoritmos/gulosa.py

import time
import heapq
from algoritmos.busca_utils import generate_valid_moves, make_move, reconstruct_path

def gulosa(estado_inicial, estado_objetivo, heuristica):
    inicio = time.time()
    tamanho = int(len(estado_inicial) ** 0.5)
    visitados = set()
    heap = []
    heapq.heappush(heap, (heuristica(estado_inicial, estado_objetivo), estado_inicial, []))
    nos_expandidos = 0

    while heap:
        h, estado_atual, caminho = heapq.heappop(heap)
        nos_expandidos += 1
        ...
        for destino in generate_valid_moves(estado_atual, tamanho):
            ...
            if tuple(novo_estado) not in visitados:
                heapq.heappush(heap, (heuristica(novo_estado, estado_objetivo), novo_estado, caminho + [estado_atual]))
